...
// ... (funções de watchedItems inalteradas)
export const getWatchedItems = async (): Promise<ManagedWatchedItem[]> => {
    ...
    querySnapshot.forEach((doc) => { items.push(doc.data() as ManagedWatchedItem); });
    return items;
};
...
// ... (funções da watchlist inalteradas)
...
// NOVO: Exporta a referência da coleção para que possamos "ouvi-la"
export const relevantReleasesCollection = collection(db, RELEASES_COLLECTION_NAME);
...
// ... (funções dos desafios inalteradas)
